...
/// [`dto::ImportStatus`] (R5).
///
/// Uses `lock().await`, not `try_lock`. An import does not hold the mutex
/// across its external awaits, so this only blocks for the brief windows where
/// begin or finish are flipping the flag. A poll therefore observes `running`
/// while an import is in flight and reads `last_summary` from the last
/// completed run, even mid-import.
#[axum::debug_handler]
